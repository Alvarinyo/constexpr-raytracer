...
SCENARIO("Multipying a matrix by the identity matrix") {
  GIVEN("The following matrix A") {
    ...
    THEN("A * identity_matrix = A") { STATIC_REQUIRE(A * identity<4>() == A); }
  }
}
...
SCENARIO("Transposing a square matrix") {
  GIVEN("The following matrix A") {
    ...
    THEN("transpose(A) is the following matrix:") {
      STATIC_REQUIRE(transpose(A) == Matrix<4>{0.f, 9.f, 1.f, 0.f, 9.f, 8.f,
                                               8.f, 0.f, 3.f, 0.f, 5.f, 5.f,
                                               0.f, 8.f, 3.f, 8.f});
    }
  }
}
...
SCENARIO("Transposing the identity matrix") {
  GIVEN("A <- transpose(A)") {
    constexpr Matrix<4> A = transpose(identity<4>());
    THEN("transpose(A) is the following matrix:") {
      STATIC_REQUIRE(A == identity<4>());
    }
  }
}
...
SCENARIO("Calculating the determinant of a 2x2 matrix") {
  GIVEN("The following 2x2 matrix A") {
    ...
    THEN("determinant(A) = 17") { STATIC_REQUIRE(determinant(A) == 17.f); }
  }
}
...
SCENARIO("A submatrix of a 3x3 matrix is a 2x2 matrix") {
  GIVEN("The following 3x3 matrix A") {
    ...
    THEN("submatrix(A, 0, 2) is the following 2x2 matrix") {
      STATIC_REQUIRE(submatrix(A, 0, 2) == Matrix<2>{-3.f, 2.f, 0.f, 6.f});
    }
  }
}
...
SCENARIO("A submatrix of a 4x4 matrix is a 3x3 matrix") {
  GIVEN("The following 4x4 matrix A") {
    ...
    THEN("submatrix(A, 2, 1) is the following 3x3 matrix") {
      STATIC_REQUIRE(submatrix(A, 2, 1) == Matrix<3>{-6.f, 1.f, 6.f, -8.f, 8.f,
                                                     6.f, -7.f, -1.f, 1.f});
    }
  }
}
...
SCENARIO("Calculating the minor of a 3x3 matrix") {
  ...
  AND_GIVEN("B <- submatrix(A, 1, 0)") {
    ...
    constexpr Matrix<2> B = submatrix(A, 1, 0);
    THEN("determinant(B) = 25") {
      STATIC_REQUIRE(approx_equal(determinant(B), 25.f));
    }
    AND_THEN("minor(A, 1, 0) = 25") {
      STATIC_REQUIRE(approx_equal(minor(A, 1, 0), 25.f));
    }
  }
}
...
SCENARIO("Calculating the cofactor of a 3x3 matrix") {
  GIVEN("The following 3x3 matrix A") {
    constexpr Matrix<3> A = {3.f, 5.f, 0.f, 2.f, -1.f, -7.f, 6.f, -1.f, 5.f};
    THEN("minor(A, 0, 0) = -12") {
      STATIC_REQUIRE(approx_equal(minor(A, 0, 0), -12.f));
    }
    AND_THEN("cofactor(A, 0, 0) = -12") {
      STATIC_REQUIRE(approx_equal(cofactor(A, 0, 0), -12.f));
    }
    AND_THEN("minor(A, 1, 0) = 25") {
      STATIC_REQUIRE(approx_equal(minor(A, 1, 0), 25.f));
    }
    AND_THEN("cofactor(A, 1, 0) = -25") {
      STATIC_REQUIRE(approx_equal(cofactor(A, 1, 0), -25.f));
    }
  }
}
...
SCENARIO("Calculating the determinant of a 3x3 matrix") {
  GIVEN("The following 3x3 matrix A") {
    ...
    THEN("cofactor(A, 0, 0) = 56") {
      STATIC_REQUIRE(approx_equal(cofactor(A, 0, 0), 56.f));
    }
    AND_THEN("cofactor(A, 0, 1) = 12") {
      STATIC_REQUIRE(approx_equal(cofactor(A, 0, 1), 12.f));
    }
    AND_THEN("minor(A, 0, 2) = -46") {
      STATIC_REQUIRE(approx_equal(cofactor(A, 0, 2), -46.f));
    }
    AND_THEN("determinant(A) = -196") {
      STATIC_REQUIRE(approx_equal(determinant(A), -196.f));
    }
  }
}
...
SCENARIO("Calculating the determinant of a 4x4 matrix") {
  GIVEN("The following 4x4 matrix A") {
    ...
    THEN("cofactor(A, 0, 0) = 56") {
      STATIC_REQUIRE(approx_equal(cofactor(A, 0, 0), 690.f));
    }
    AND_THEN("cofactor(A, 0, 1) = 12") {
      STATIC_REQUIRE(approx_equal(cofactor(A, 0, 1), 447.f));
    }
    AND_THEN("minor(A, 0, 2) = -46") {
      STATIC_REQUIRE(approx_equal(cofactor(A, 0, 2), 210.f));
    }
    AND_THEN("minor(A, 0, 3) = 51") {
      STATIC_REQUIRE(approx_equal(cofactor(A, 0, 3), 51.f));
    }
    AND_THEN("determinant(A) = -4071") {
      STATIC_REQUIRE(approx_equal(determinant(A), -4071.f));
    }
  }
}
...
SCENARIO("Testing an invertible matrix for invertibility") {
  GIVEN("The following 4x4 matrix A") {
    ...
    THEN("determinat(A) = -2120") {
      STATIC_REQUIRE(approx_equal(determinant(A), -2120.f));
    }
    AND_THEN("A is invertible") { STATIC_REQUIRE(is_invertible(A)); }
  }
}
...
SCENARIO("Testing an noninvertible matrix for invertibility") {
  GIVEN("The following 4x4 matrix A") {
    ...
    THEN("determinat(A) = 0") {
      STATIC_REQUIRE(approx_equal(determinant(A), 0.f));
    }
    AND_THEN("A is not invertible") { STATIC_REQUIRE(!is_invertible(A)); }
  }
}
...
SCENARIO("Calculating the inverse of a matrix") {
  ...
  AND_GIVEN("B <- inverse(A)") {
    ...
    constexpr auto B = inverse(A);
    THEN("determinat(A) = 532") {
      STATIC_REQUIRE(approx_equal(determinant(A), 532.f));
    }
    AND_THEN("cofactor(A, 2, 3) = -160") {
      STATIC_REQUIRE(approx_equal(cofactor(A, 2, 3), -160.f));
    }
    AND_THEN("B.at(3, 2) = -160/532") {
      STATIC_REQUIRE(approx_equal(B.at(3, 2), -160.f / 532.f));
    }
    AND_THEN("cofactor(A, 3, 2) = -105") {
      STATIC_REQUIRE(approx_equal(cofactor(A, 3, 2), 105.f));
    }
...
SCENARIO("Multiplying a product by its inverse") {
  ...
  AND_GIVEN("C <- A*B") {
    ...
    THEN("C * inverse(B) = A") { STATIC_REQUIRE(C * inverse(B) == A); }
  }
}